...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier forSale(uint _sku){
    require(items[_sku].state == State.ForSale && items[_sku].seller != address(0),"item not in sale");
    _;
  }
  modifier sold(uint _sku){
    require(items[_sku].state == State.Sold && items[_sku].buyer != address(0),"item not sold yet");
    _;
  }
  ...
  forSale(sku) paidEnough(items[sku].price) checkValue(sku)  public 
  {
    Item storage itm = items[sku];
    itm.buyer = msg.sender;
    itm.seller.transfer(msg.value);
    itm.state = State.Sold;
     emit LogSold(sku);
